...
class Connection:
    ...
    def _build_oauth2_session(self, client_id, token=None, redirect_uri=None, scopes=None, state=None):
        ...
        the app initiates the session, redirects to MS, then receives the
        authenticated code on a callback URL).

        :param str client_id: app client id
        :param str redirect_uri: the redirect url to provide,
                or that was originally provided
        :param list[str] scopes: scopes to request
        :param str state: anti-CSRF code generated by a previous request
    ...
    def get_authorization_details(self):
        """ Return details on the last authorization-url generation.
        These are necessary to recreate a session in webapp flows.

        :return: (state, redirect_uri, scopes)
        :rtype: tuple

        """
        if self.session is None or not getattr(self.session, "state", False):
            raise RuntimeError("First call 'get_authorization_url' to "
                               "generate a valid oauth object")
        return self.session.state, self.session.redirect_uri, self.session.scope

    def get_authorization_webflow(self, requested_scopes=None,
                              redirect_uri=OAUTH_REDIRECT_URL, **kwargs):
        """ Generate an authorization url with explicit state, and return all the
        related details. This is typically used in webapp flows, where the
        receiving callback endpoint needs to reconcile requests with originating calls.

        :param list[str] requested_scopes: list of scopes to request access for
        :param str redirect_uri: redirect url configured in registered app
        :param kwargs: allow to pass unused params in conjunction with Connection
        :return: (authorization_url, state, redirect_uri, scope)
        :rtype: tuple

        """
        url = self.get_authorization_url(requested_scopes, redirect_uri, **kwargs)
        details = self.get_authorization_details()
        return (url, *details)

    def recreate_session(self, state, redirect_uri=OAUTH_REDIRECT_URL, scopes=None):
        """ reconstruct a previously-initialized OAuth2 session

        :param str state: anti-csrf code
        :param str redirect_uri: the redirect URL submitted with the original request
    ...
    def request_token(self, authorization_url, store_token=True,
        ...
        :param str state: anti-csrf code generated in previous calls.
                            if we want to rebuild the session
        :param kwargs: allow to pass unused params in conjunction with Connection
        :return: Success/Failure
        :rtype: bool
        """

        if self.session is None and state is None:
            raise RuntimeError("Fist call 'get_authorization_url' to "
                               "generate a valid oauth object")

        client_id, client_secret = self.auth
        if state is not None:  # rebuild the session with the passed state
            self.session = self._build_oauth2_session(client_id=client_id,redirect_uri=redirect_uri,scopes=scopes)
