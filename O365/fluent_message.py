...
class Message(object):
    ...
    def __init__(self, json=None, auth=None, verify=True, oauth=None):
        ...
        if self.oauth:
            self.att_url = self.att_url.replace(
                'https://outlook.office365.com/api',
                'https://graph.microsoft.com')
            self.send_url = self.send_url.replace(
                'https://outlook.office365.com/api',
                'https://graph.microsoft.com')
            self.send_as_url = self.send_as_url.replace(
                'https://outlook.office365.com/api',
                'https://graph.microsoft.com')
            self.draft_url = self.draft_url.replace(
                'https://outlook.office365.com/api',
    ...
    def fetchAttachments(self, **kwargs):
        ...
        response = (self.oauth, requests)[self.oauth is None].get(
            self.att_url.format(
        ...
        for att in json['value']:
            try:
                ...
                log.debug('successfully downloaded attachment for: %s.',
                          self.auth[0])
    ...
    def sendMessage(self, user_id=None, **kwargs):
        ...
        response = (self.oauth, requests)[self.oauth is None].post(
            url, data, headers=headers, auth=self.auth, verify=self.verify,
            **kwargs)
    ...
    def markAsRead(self):
        ...
        try:
            response = (self.oauth, requests)[self.oauth is None].patch(
    ...
    def update_category(self, category_name, **kwargs):
        ...
        try:
            response = (self.oauth, requests)[self.oauth is None].patch(
                self.update_url.format(
                    self.json['Id']), category, headers=headers, auth=self.auth,
